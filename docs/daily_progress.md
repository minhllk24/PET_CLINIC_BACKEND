...
- **Bổ sung và nâng cấp các API cho màn hình Đơn hàng của tôi (My Orders) và Chi tiết đơn hàng:**
  ...
  - Bổ sung quan hệ `status_history` sắp xếp tăng dần theo thời gian trong `getOrderById` để phục vụ hiển thị mốc thời gian trên thanh tiến trình (Stepper) của chi tiết đơn hàng.
