"use strict";

var _prisma = _interopRequireDefault(require("../configs/prisma"));
var _prismaHelpers = require("../utils/prismaHelpers");
function _interopRequireDefault(e) { return e && e.__esModule ? e : { "default": e }; }
function _regenerator() { /*! regenerator-runtime -- Copyright (c) 2014-present, Facebook, Inc. -- license (MIT): https://github.com/babel/babel/blob/main/packages/babel-helpers/LICENSE */ var e, t, r = "function" == typeof Symbol ? Symbol : {}, n = r.iterator || "@@iterator", o = r.toStringTag || "@@toStringTag"; function i(r, n, o, i) { var c = n && n.prototype instanceof Generator ? n : Generator, u = Object.create(c.prototype); return _regeneratorDefine2(u, "_invoke", function (r, n, o) { var i, c, u, f = 0, p = o || [], y = !1, G = { p: 0, n: 0, v: e, a: d, f: d.bind(e, 4), d: function d(t, r) { return i = t, c = 0, u = e, G.n = r, a; } }; function d(r, n) { for (c = r, u = n, t = 0; !y && f && !o && t < p.length; t++) { var o, i = p[t], d = G.p, l = i[2]; r > 3 ? (o = l === n) && (u = i[(c = i[4]) ? 5 : (c = 3, 3)], i[4] = i[5] = e) : i[0] <= d && ((o = r < 2 && d < i[1]) ? (c = 0, G.v = n, G.n = i[1]) : d < l && (o = r < 3 || i[0] > n || n > l) && (i[4] = r, i[5] = n, G.n = l, c = 0)); } if (o || r > 1) return a; throw y = !0, n; } return function (o, p, l) { if (f > 1) throw TypeError("Generator is already running"); for (y && 1 === p && d(p, l), c = p, u = l; (t = c < 2 ? e : u) || !y;) { i || (c ? c < 3 ? (c > 1 && (G.n = -1), d(c, u)) : G.n = u : G.v = u); try { if (f = 2, i) { if (c || (o = "next"), t = i[o]) { if (!(t = t.call(i, u))) throw TypeError("iterator result is not an object"); if (!t.done) return t; u = t.value, c < 2 && (c = 0); } else 1 === c && (t = i["return"]) && t.call(i), c < 2 && (u = TypeError("The iterator does not provide a '" + o + "' method"), c = 1); i = e; } else if ((t = (y = G.n < 0) ? u : r.call(n, G)) !== a) break; } catch (t) { i = e, c = 1, u = t; } finally { f = 1; } } return { value: t, done: y }; }; }(r, o, i), !0), u; } var a = {}; function Generator() {} function GeneratorFunction() {} function GeneratorFunctionPrototype() {} t = Object.getPrototypeOf; var c = [][n] ? t(t([][n]())) : (_regeneratorDefine2(t = {}, n, function () { return this; }), t), u = GeneratorFunctionPrototype.prototype = Generator.prototype = Object.create(c); function f(e) { return Object.setPrototypeOf ? Object.setPrototypeOf(e, GeneratorFunctionPrototype) : (e.__proto__ = GeneratorFunctionPrototype, _regeneratorDefine2(e, o, "GeneratorFunction")), e.prototype = Object.create(u), e; } return GeneratorFunction.prototype = GeneratorFunctionPrototype, _regeneratorDefine2(u, "constructor", GeneratorFunctionPrototype), _regeneratorDefine2(GeneratorFunctionPrototype, "constructor", GeneratorFunction), GeneratorFunction.displayName = "GeneratorFunction", _regeneratorDefine2(GeneratorFunctionPrototype, o, "GeneratorFunction"), _regeneratorDefine2(u), _regeneratorDefine2(u, o, "Generator"), _regeneratorDefine2(u, n, function () { return this; }), _regeneratorDefine2(u, "toString", function () { return "[object Generator]"; }), (_regenerator = function _regenerator() { return { w: i, m: f }; })(); }
function _regeneratorDefine2(e, r, n, t) { var i = Object.defineProperty; try { i({}, "", {}); } catch (e) { i = 0; } _regeneratorDefine2 = function _regeneratorDefine(e, r, n, t) { function o(r, n) { _regeneratorDefine2(e, r, function (e) { return this._invoke(r, n, e); }); } r ? i ? i(e, r, { value: n, enumerable: !t, configurable: !t, writable: !t }) : e[r] = n : (o("next", 0), o("throw", 1), o("return", 2)); }, _regeneratorDefine2(e, r, n, t); }
function asyncGeneratorStep(n, t, e, r, o, a, c) { try { var i = n[a](c), u = i.value; } catch (n) { return void e(n); } i.done ? t(u) : Promise.resolve(u).then(r, o); }
function _asyncToGenerator(n) { return function () { var t = this, e = arguments; return new Promise(function (r, o) { var a = n.apply(t, e); function _next(n) { asyncGeneratorStep(a, r, o, _next, _throw, "next", n); } function _throw(n) { asyncGeneratorStep(a, r, o, _next, _throw, "throw", n); } _next(void 0); }); }; }
// --- VOUCHERS ---
var getAllVouchers = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee() {
    var vouchers, _t;
    return _regenerator().w(function (_context) {
      while (1) switch (_context.p = _context.n) {
        case 0:
          _context.p = 0;
          _context.n = 1;
          return _prisma["default"].voucher.findMany({
            orderBy: {
              created_at: 'desc'
            }
          });
        case 1:
          vouchers = _context.v;
          return _context.a(2, {
            EM: 'Get vouchers successful',
            EC: 0,
            DT: vouchers
          });
        case 2:
          _context.p = 2;
          _t = _context.v;
          console.error(_t);
          return _context.a(2, {
            EM: 'Something went wrong',
            EC: -2,
            DT: ''
          });
      }
    }, _callee, null, [[0, 2]]);
  }));
  return function getAllVouchers() {
    return _ref.apply(this, arguments);
  };
}();
var createVoucher = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2(data) {
    var _data$code, code, existing, newVoucher, _t2;
    return _regenerator().w(function (_context2) {
      while (1) switch (_context2.p = _context2.n) {
        case 0:
          _context2.p = 0;
          code = (_data$code = data.code) === null || _data$code === void 0 ? void 0 : _data$code.toUpperCase();
          if (code) {
            _context2.n = 1;
            break;
          }
          return _context2.a(2, {
            EM: 'Missing voucher code',
            EC: 1,
            DT: ''
          });
        case 1:
          _context2.n = 2;
          return _prisma["default"].voucher.findUnique({
            where: {
              code: code
            }
          });
        case 2:
          existing = _context2.v;
          if (!existing) {
            _context2.n = 3;
            break;
          }
          return _context2.a(2, {
            EM: 'Voucher code already exists',
            EC: 1,
            DT: ''
          });
        case 3:
          _context2.n = 4;
          return _prisma["default"].voucher.create({
            data: {
              code: code,
              discount_type: data.discount_type || 'percentage',
              discount_value: parseFloat(data.discount_value) || 0,
              max_discount: data.max_discount ? parseFloat(data.max_discount) : null,
              min_order_value: data.min_order_value ? parseFloat(data.min_order_value) : null,
              start_date: data.start_date ? new Date(data.start_date) : new Date(),
              end_date: data.end_date ? new Date(data.end_date) : null,
              usage_limit: data.usage_limit ? parseInt(data.usage_limit) : null,
              status: data.status || 'active'
            }
          });
        case 4:
          newVoucher = _context2.v;
          return _context2.a(2, {
            EM: 'Create voucher successful',
            EC: 0,
            DT: newVoucher
          });
        case 5:
          _context2.p = 5;
          _t2 = _context2.v;
          console.error(_t2);
          return _context2.a(2, {
            EM: 'Something went wrong',
            EC: -2,
            DT: ''
          });
      }
    }, _callee2, null, [[0, 5]]);
  }));
  return function createVoucher(_x) {
    return _ref2.apply(this, arguments);
  };
}();
var applyVoucher = /*#__PURE__*/function () {
  var _ref3 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee3(userIdStr, data) {
    var _data$code2, userId, code, orderValue, voucher, now, userUsed, discountAmount, _t3;
    return _regenerator().w(function (_context3) {
      while (1) switch (_context3.p = _context3.n) {
        case 0:
          _context3.p = 0;
          userId = (0, _prismaHelpers.toBigIntId)(userIdStr);
          code = (_data$code2 = data.code) === null || _data$code2 === void 0 ? void 0 : _data$code2.toUpperCase();
          orderValue = parseFloat(data.order_value);
          if (!(!code || isNaN(orderValue))) {
            _context3.n = 1;
            break;
          }
          return _context3.a(2, {
            EM: 'Missing code or order_value',
            EC: 1,
            DT: ''
          });
        case 1:
          _context3.n = 2;
          return _prisma["default"].voucher.findUnique({
            where: {
              code: code
            }
          });
        case 2:
          voucher = _context3.v;
          if (!(!voucher || voucher.status !== 'active')) {
            _context3.n = 3;
            break;
          }
          return _context3.a(2, {
            EM: 'Voucher not valid or inactive',
            EC: -1,
            DT: ''
          });
        case 3:
          now = new Date();
          if (!(voucher.start_date && now < voucher.start_date)) {
            _context3.n = 4;
            break;
          }
          return _context3.a(2, {
            EM: 'Voucher not yet active',
            EC: -1,
            DT: ''
          });
        case 4:
          if (!(voucher.end_date && now > voucher.end_date)) {
            _context3.n = 5;
            break;
          }
          return _context3.a(2, {
            EM: 'Voucher expired',
            EC: -1,
            DT: ''
          });
        case 5:
          if (!(voucher.min_order_value && orderValue < parseFloat(voucher.min_order_value))) {
            _context3.n = 6;
            break;
          }
          return _context3.a(2, {
            EM: "Minimum order value is ".concat(voucher.min_order_value),
            EC: -1,
            DT: ''
          });
        case 6:
          if (!(voucher.usage_limit && voucher.used_count >= voucher.usage_limit)) {
            _context3.n = 7;
            break;
          }
          return _context3.a(2, {
            EM: 'Voucher usage limit reached',
            EC: -1,
            DT: ''
          });
        case 7:
          _context3.n = 8;
          return _prisma["default"].voucherUsage.findFirst({
            where: {
              user_id: userId,
              voucher_id: voucher.voucher_id
            }
          });
        case 8:
          userUsed = _context3.v;
          if (!userUsed) {
            _context3.n = 9;
            break;
          }
          return _context3.a(2, {
            EM: 'You have already used this voucher',
            EC: -1,
            DT: ''
          });
        case 9:
          discountAmount = 0;
          if (voucher.discount_type === 'percentage') {
            discountAmount = orderValue * (parseFloat(voucher.discount_value) / 100);
            if (voucher.max_discount && discountAmount > parseFloat(voucher.max_discount)) {
              discountAmount = parseFloat(voucher.max_discount);
            }
          } else {
            discountAmount = parseFloat(voucher.discount_value);
          }
          return _context3.a(2, {
            EM: 'Voucher valid',
            EC: 0,
            DT: {
              discountAmount: discountAmount,
              voucher_id: voucher.voucher_id
            }
          });
        case 10:
          _context3.p = 10;
          _t3 = _context3.v;
          console.error(_t3);
          return _context3.a(2, {
            EM: 'Something went wrong',
            EC: -2,
            DT: ''
          });
      }
    }, _callee3, null, [[0, 10]]);
  }));
  return function applyVoucher(_x2, _x3) {
    return _ref3.apply(this, arguments);
  };
}();

// --- LOYALTY ---
var getMyPoints = /*#__PURE__*/function () {
  var _ref4 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee4(userIdStr) {
    var userId, user, _t4;
    return _regenerator().w(function (_context4) {
      while (1) switch (_context4.p = _context4.n) {
        case 0:
          _context4.p = 0;
          userId = (0, _prismaHelpers.toBigIntId)(userIdStr);
          _context4.n = 1;
          return _prisma["default"].user.findUnique({
            where: {
              user_id: userId
            },
            select: {
              loyalty_points: true
            }
          });
        case 1:
          user = _context4.v;
          return _context4.a(2, {
            EM: 'Get points successful',
            EC: 0,
            DT: {
              points: user.loyalty_points
            }
          });
        case 2:
          _context4.p = 2;
          _t4 = _context4.v;
          console.error(_t4);
          return _context4.a(2, {
            EM: 'Something went wrong',
            EC: -2,
            DT: ''
          });
      }
    }, _callee4, null, [[0, 2]]);
  }));
  return function getMyPoints(_x4) {
    return _ref4.apply(this, arguments);
  };
}();
var getMyTransactions = /*#__PURE__*/function () {
  var _ref5 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee5(userIdStr) {
    var userId, trans, _t5;
    return _regenerator().w(function (_context5) {
      while (1) switch (_context5.p = _context5.n) {
        case 0:
          _context5.p = 0;
          userId = (0, _prismaHelpers.toBigIntId)(userIdStr);
          _context5.n = 1;
          return _prisma["default"].loyaltyPointTransaction.findMany({
            where: {
              user_id: userId
            },
            orderBy: {
              created_at: 'desc'
            }
          });
        case 1:
          trans = _context5.v;
          return _context5.a(2, {
            EM: 'Get point transactions successful',
            EC: 0,
            DT: trans
          });
        case 2:
          _context5.p = 2;
          _t5 = _context5.v;
          console.error(_t5);
          return _context5.a(2, {
            EM: 'Something went wrong',
            EC: -2,
            DT: ''
          });
      }
    }, _callee5, null, [[0, 2]]);
  }));
  return function getMyTransactions(_x5) {
    return _ref5.apply(this, arguments);
  };
}();
module.exports = {
  getAllVouchers: getAllVouchers,
  createVoucher: createVoucher,
  applyVoucher: applyVoucher,
  getMyPoints: getMyPoints,
  getMyTransactions: getMyTransactions
};